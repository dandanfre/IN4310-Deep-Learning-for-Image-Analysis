...
class ResNet(nn.Module):
    ...
    def __init__(
        self, img_channels: int, num_layers: int = 18, num_classes: int = 1000
    ):
        ...
        if num_layers == 18:
            ...
            # The expansion number is 1 -> uses BasicBlock
            self.expansion = 1
            ...
            # Number of blocks per stage for ResNet-34 is 3,4,6,3 per stage.
            ...
            # ResNet-50 uses bottleneck blocks with 3, 4, 6, and 3 blocks per stage
            block_counts = [3, 4, 6, 3]

            # 2)Bottleneck blocks have an expansion factor of 4.
            self.expansion = 4
            # 3) Set the block type to BottleneckBlock.
            blockType = BottleneckBlock

        elif num_layers == 101:
            # 1) ResNet-101 uses bottleneck blocks with 3, 4, 23, and 3 blocks per stage.
            ...
            # TODO:
            # ResNet-152 uses bottleneck blocks with 3, 8, 36, and 3 blocks per stage.
            block_counts = [3, 8, 36, 3]
            # Bottleneck blocks have an expansion factor of 4.
            self.expansion = 4
            # Set the block type to BottleneckBlock.
            blockType = BottleneckBlock

        else:
            raise ValueError(
                "Only ResNet-18, ResNet-34, ResNet-50, ResNet-101, and ResNet-152 are supported in this implementation."
            )
    ...
    def build_stage(
        self, out_channels: int, num_blocks: int, stride: int, block
    ) -> nn.Sequential:
        """
