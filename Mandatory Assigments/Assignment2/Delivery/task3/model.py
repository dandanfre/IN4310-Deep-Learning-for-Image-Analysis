...
class ImageCaptionModel(nn.Module):
    ...
    def forward(self, cnn_features, token_ids, is_train):
        ...
        if self.use_attention:
            # Permute features: from [batch, channels, H, W] to [batch, H, W, channels],
            #       then flatten spatial dimensions to get [batch, num_regions, channels]
            cnn_features = cnn_features.permute(0, 2, 3, 1)  # [batch, H, W, channels]
            cnn_features = cnn_features.view(
                cnn_features.size(0), -1, cnn_features.size(3)
            )
            # Apply the projection to each region (nn.Linear applies to the last dim).
            processed_img_feat = self.feature_projection(
                cnn_features
            )  # [batch, num_regions, hidden_size]
...
class CaptionRNN(nn.Module):
    ...
    def forward(self, tokens, features, is_train):
        ...
        # Initialize hidden_states with correct dimensions depending on the cell type.
        ...
        for t in range(seq_len):
            ...
            for layer in range(self.num_layers):
                if layer == 0:
                    if self.use_attention:
                        # Compute attention: use previous hidden state to attend over features.
                        prev_hidden = hidden_states[layer][:, : self.hidden_state_size]
                        context, alpha = self.attention(features, prev_hidden)
                        attn_weights_series.append(alpha)
                        # Concatenate token embedding with the attended image context.
                        cell_input = torch.cat([current_token_vec, context], dim=1)
                    else:
                        # Without attention, concatenate the token embedding with the image feature.
                        cell_input = torch.cat([current_token_vec, features], dim=1)
                else:
                    # For higher layers, input is the hidden state from the previous layer
                    # Only use the first half of the hidden state
                ...
                if layer == self.num_layers - 1:
                    # Get logits from the self.output_layer and append them to logits_series
...
class LSTMCell(nn.Module):
    def __init__(self, input_size: int, hidden_state_size: int):
        ...
        # Initialise weights and biases for the forget gate (weight_f, bias_f), input gate (w_i, b_i),
    ...
    def forward(self, x, hidden_state):
        ...
        """
        # Implement the LSTM equations to get the new hidden state, cell memory and return them.
...
class Attention(nn.Module):
    def __init__(self, cnn_feature_dim, hidden_dim):
        ...
        # Create layers to project the hidden state and the image features to a common dimension.
        self.hidden_to_hidden = nn.Linear(
            hidden_dim, hidden_dim
        )  # Linear layer to project hidden_state to hidden_dim
        self.features_to_hidden = nn.Linear(
            cnn_feature_dim, hidden_dim
        )  # Linear layer to project cnn_feature_dim to hidden_dim
    ...
    def forward(self, features, hidden_state):
        ...
        """
        # Project hidden state and unsqueeze() to [batch, 1, hidden_dim] so that it can be broadcast.
        hidden_proj = self.hidden_to_hidden(hidden_state).unsqueeze(
            1
        )  # [batch, 1, hidden_dim]

        # Project encoder outputs
        enc_proj = self.features_to_hidden(features)  # [batch, num_regions, hidden_dim]

        # Add the projections and apply self.relu
        att = self.relu(enc_proj + hidden_proj)  # [batch, num_regions, hidden_dim]

        # Get scalar attention scores for each region of the image
        att_scores = self.hidden_to_attention_score(att).squeeze(
            2
        )  # [batch, num_regions]

        # Apply softmax on the att_scores to get the alphas (attention weights)
        alpha = self.softmax(att_scores)  # [batch, num_regions]

        # Compute context vector as the weighted sum of encoder outputs. You might need to unsqueeze() alpha.
        context = (features * alpha.unsqueeze(2)).sum(dim=1)  # [batch, encoder_dim]
