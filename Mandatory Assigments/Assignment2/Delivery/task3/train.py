...
def train():
    ...
    train_dataset = COCODataset(
        config.train_caption_file, config.resnet50_features_train_file, config, True
    )
    val_dataset = COCODataset(
        config.val_caption_file, config.resnet50_features_val_file, config, False
    )
    ...
    criterion = nn.CrossEntropyLoss(
        ignore_index=train_dataset.vocab.word2idx["<pad>"]
    ).to(device)
    ...
    model_name = (
        f"{os.environ['USER']}_{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}.pth"
    )
    ...
    for epoch in range(config.num_epochs):
        ...
        for features, captions, image_ids in train_loader:
            ...
            input_tokens = captions[:, :-1].to(
                device
            )  # (batch_size, max_caption_length - 1)
            target_tokens = captions[:, 1:].to(
                device
            )  # (batch_size, max_caption_length - 1)

            # Forward pass.
            outputs, alphas = model(
                features, input_tokens, is_train=True
        ...
        print(
            f"Epoch {epoch} took {timedelta(seconds=time.time() - epoch_start)}",
            flush=True,
        )
        scheduler.step()
        print(
            f"Epoch [{epoch + 1}/{config.num_epochs}], Loss: {running_loss / (len(train_dataset)): .4f}",
            flush=True,
        )

        # Evaluate the model on the validation data
        eval_start = time.time()
        metric_scores, generated_captions = evaluate_model(
            val_loader, model, device, val_metrics
        )
        print(
            f"Eval after epoch {epoch} took {timedelta(seconds=time.time() - eval_start)}",
            flush=True,
        )
        ...
        # if metric_scores[Bleu().method()][0][3] > best_bleu4:
        #    best_bleu4 = metric_scores[Bleu().method()][0][3]  # BLEU-4 score
        #   print("New best BLEU score is", best_bleu4)
    ...
    plot_loss(
        f"plots/loss_plot_{timestamp}.jpg",
        step_losses,
        epoch_start_steps,
    )
    plot_metrics(
        f"plots/metric_{timestamp}.jpg",
...
def debug():
    ...
    model = ImageCaptionModel(
        feature_dim, embed_size, hidden_size, vocab_size, 7, 2, "LSTM"
    )
    features = torch.randn(3, feature_dim, 3, 3)
    captions = torch.randint(0, 5, (3, 7))
    print(model(features, captions, is_train=True))
